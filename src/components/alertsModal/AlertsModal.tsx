  ...
  const renderErrors = (errors: string[]): JSX.Element => {
    return errors.length ? (
      <div className="errors">
        <h4 className="errors-header">The following errors appeared while running:</h4>
  ...
  const renderWarnings = (warnings: string[]): JSX.Element => {
    return warnings.length ? (
      <div className="warnings">
        <h4 className="warnings-header">The following warnings appeared while running:</h4>
        <ul>
          {warnings.map((w) => (
